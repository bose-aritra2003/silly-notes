...
const NotesPage = async () => {
  ...
  return (
    <div className={"px-10 md:px-20 mt-10 space-y-12"}>
      ...
      <div
        className={"grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-10 xl:gap-12 2xl:gap-16"}>
        {
          notes?.map((note: any) => {
...
const Note = ({note}: any) => {
  ...
  return (
    <Link id={id} href={`/notes/${id}`}>
      <div
        className={"bg-yellow-300 space-y-5 shadow-xl p-5 cursor-pointer transition ease-in-out hover:shadow-2xl hover:border-black hover:border-2"}>
